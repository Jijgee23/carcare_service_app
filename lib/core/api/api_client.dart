import 'package:carcare_service/core/api/token_interceptor.dart';
...
/// Safely extracts `error` field from a response body that may be a Map,
/// List, or String — avoids `type 'String' is not a subtype of type 'int'`.
String? _bodyError(dynamic data) {
...
AppError _toAppError(DioException e) {
  return switch (e.type) {
    ...
    DioExceptionType.receiveTimeout => const AppError(ErrorKind.network, 'Холболт байхгүй'),
    DioExceptionType.badResponse => switch (e.response?.statusCode) {
      401 => const AppError(ErrorKind.unauthorized, 'Нэвтрэх шаардлагатай'),
      403 => AppError(ErrorKind.forbidden, _bodyError(e.response?.data) ?? 'Хандах эрх байхгүй'),
      404 => const AppError(ErrorKind.notFound, 'Олдсонгүй'),
      500 => const AppError(ErrorKind.server, 'Серверийн алдаа'),
      _ => AppError(ErrorKind.unknown, _bodyError(e.response?.data) ?? 'Алдаа гарлаа'),
    },
    _ => AppError(ErrorKind.unknown, e.message ?? 'Алдаа гарлаа'),
  };
}

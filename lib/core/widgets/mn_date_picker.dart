...
/// Огнооны хязгаар сонгогч (эхлэх/дуусах). Returns null if dismissed.
///
/// First tap sets the start; a second tap sets the end (if the tap lands
/// before the current start, it becomes the new start and any previous end
/// is cleared). Tapping the same day twice yields a single-day range.
/// Returned dates are date-only (midnight local).
Future<DateTimeRange?> showMnDateRangePicker(
  BuildContext context, {
...
class _MnRangePickerSheet extends StatefulWidget {
  final DateTimeRange? initialRange;
  final DateTime? firstDate;
  final DateTime? lastDate;

  const _MnRangePickerSheet({
    this.initialRange,
    this.firstDate,
    this.lastDate,
  });
  ...
  @override
  State<_MnRangePickerSheet> createState() => _MnRangePickerSheetState();
}

class _MnRangePickerSheetState extends State<_MnRangePickerSheet> {
  late int _year;
  late int _month;
  DateTime? _start;
  DateTime? _end;
  ...
  void initState() {
    ...
    if (initial != null) {
      ...
      _end = DateTime(initial.end.year, initial.end.month, initial.end.day);
    }
  }
  ...
  String get _summary {
    if (_start == null) return 'Эхлэх өдрөө сонгоно уу';
    if (_end == null) return '${_fmt(_start!)}  →  ...';
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // brandSurface is a fixed always-dark surface regardless of app
          // brightness, so the literal Colors.white/white38/white54/etc.
          // below (not brightness-following ink tokens) are intentional.
          Container(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          color: inRange
                              ? context.colors.accent.withOpacity(0.18)
                              : Colors.transparent,
                          borderRadius: BorderRadius.horizontal(
                            left: inRange && isStart
                ...
                Expanded(
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: Text(
                        canConfirm ? 'Сонгох' : 'Огноо сонгоно уу',
                        style: const TextStyle(
                          fontSize: 13,
                          fontWeight: FontWeight.w700,
                        ),

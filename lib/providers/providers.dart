...
class InspectionProvider extends ChangeNotifier {
  ...
  int get todayCount => _records.where((r) => _isToday(r.date)).length;
  int get totalToday => 12;

  void setTab(int tab) {
    _currentTab = tab;
    switch (tab) {
      case 0: _filterStatus = null; break;
      case 1: _filterStatus = CheckStatus.good; break;
      case 2: _filterStatus = CheckStatus.warning; break;
      case 3: _filterStatus = CheckStatus.danger; break;
    }
    notifyListeners();
  }
  ...
  Map<String, int> get weeklyStats => {
    '05/14': 8, '05/15': 12, '05/16': 6, '05/17': 15,
    '05/18': 9, '05/19': 18, '05/20': 16,
  };

  int get totalGood => _records.fold(0, (s, r) => s + r.goodCount);
  int get totalWarning => _records.fold(0, (s, r) => s + r.warningCount);
  int get totalDanger => _records.fold(0, (s, r) => s + r.dangerCount);
}
...
class NewInspectionProvider extends ChangeNotifier {
  ...
  void nextSection() { if (canGoNext) { _currentSection++; notifyListeners(); } }
  void prevSection() { if (canGoPrev) { _currentSection--; notifyListeners(); } }
  ...
  void updateItemStatus(String sectionId, String itemId, CheckStatus status) {
    ...
    if (itemIdx == -1) return;
    _sections[secIdx].items[itemIdx] = _sections[secIdx].items[itemIdx].copyWith(status: status);
    notifyListeners();
  }

  void addPhoto(String url) { _photoUrls = [..._photoUrls, url]; notifyListeners(); }

  InspectionRecord buildRecord() => InspectionRecord(
        id: DateTime.now().millisecondsSinceEpoch.toString(),
        date: _date, plateNumber: _plateNumber,
        vehicleName: '$_make $_model $_year', type: _type, mileage: _mileage,
        sections: _sections, photoUrls: _photoUrls,
        comment: _comment.isNotEmpty ? _comment : null,
        recommendation: _recommendation.isNotEmpty ? _recommendation : null,
      );

  void reset() {
    _plateNumber = ''; _make = ''; _model = ''; _vin = ''; _mileage = 0;
    _comment = ''; _recommendation = ''; _currentSection = 0; _photoUrls = [];
    _sections = SampleData.defaultSections
        .map((s) => InspectionSection(
              id: s.id, title: s.title,
              items: s.items.map((i) => CheckItem(id: i.id, name: i.name)).toList(),
...
class BottomNavProvider extends ChangeNotifier {
  ...
  void setIndex(int i) { _index = i; notifyListeners(); }
}

    ...
    return AppCard(
      ...
      child: Row(
        ...
        children: [
          Container(
            width: 3, height: 56,
            decoration: BoxDecoration(color: record.overallStatus.color, borderRadius: BorderRadius.circular(2)),
          ),
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(record.vehicleName, style: AppTextStyles.caption.copyWith(color: AppColors.textSecondary)),
            ]),
          ),
...
class QuickActionCard extends StatelessWidget {
  ...
  final Color bgColor;
  final VoidCallback? onTap;
  const QuickActionCard({super.key, required this.icon, required this.label, required this.iconColor, required this.bgColor, this.onTap});
  ...
  Widget build(BuildContext context) {
    return AppCard(
      ...
      child: Column(children: [
        ...
        Text(label, style: AppTextStyles.captionMedium, textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis),
      ]),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Өнөөдрийн үзлэг', style: AppTextStyles.caption.copyWith(color: Colors.white60)),
        ...
        Container(
          width: 110, height: 70,
          decoration: BoxDecoration(color: Colors.white.withOpacity(0.08), borderRadius: BorderRadius.circular(AppDimens.radiusMD)),
          ...
          if (onAddTap != null)
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: AppColors.background, borderRadius: BorderRadius.circular(AppDimens.radiusMD),
                  border: Border.all(color: AppColors.divider, width: 1.5),
                ),
                child: const Icon(Icons.camera_alt_outlined, size: 28, color: AppColors.textHint),
              ),
            ),
          ...urls.map((url) => Container(
                width: 80, height: 80,
                margin: const EdgeInsets.only(right: 8),
                decoration: BoxDecoration(borderRadius: BorderRadius.circular(AppDimens.radiusMD), color: AppColors.divider),
                child: ClipRRect(
                  borderRadius: BorderRadius.circular(AppDimens.radiusMD),
                  child: Image.network(url, fit: BoxFit.cover,
                      errorBuilder: (context, error, stackTrace) => const Icon(Icons.broken_image, color: AppColors.textHint)),
                ),
              )),
        ],
      ),

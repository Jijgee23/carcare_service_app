...
class DiagnosticRepository {
  ...
  Future<DiagnosticTemplateDetail?> getTemplateDetail(String id) async {
    ...
    return DiagnosticTemplateDetail.fromJson(res.data['template'] as Map<String, dynamic>);
  }
  ...
  Future<PagedResult<DiagnosticReportSummary>> getReports({
    ...
    if (res == null) {
      return PagedResult(items: [], pagination: PaginationMeta.empty(page, pageSize));
    }
    final list = res.data['reports'] as List;
    final meta = PaginationMeta.fromJson(res.data['pagination'] as Map<String, dynamic>);
    return PagedResult(
      items: list.map((e) => DiagnosticReportSummary.fromJson(e as Map<String, dynamic>)).toList(),
      pagination: meta,
    );
  }
  ...
  Future<DiagnosticReportDetail?> getReportDetail(String id) async {
    ...
    if (res == null) return null;
    return DiagnosticReportDetail.fromJson(res.data['report'] as Map<String, dynamic>);
  }
  ...
  Future<Result<DiagnosticTemplateSummary>> createTemplate({
    ...
    try {
      ...
      return Ok(DiagnosticTemplateSummary.fromJson(res.data['template'] as Map<String, dynamic>));
    } on AppError catch (e) {
      return Err(e);
    }
  }
  ...
  Future<String?> submitReport(FormData formData) async {
    try {
      final res = await ApiService.instance.dio.post('diagnostics/reports', data: formData);
      if (res.statusCode == 201 && res.data != null) {
        return res.data['report']['id'] as String?;
      }
    } on DioException catch (e) {
      final msg = e.response?.data?['error']?.toString() ?? 'Тайлан илгээхэд алдаа гарлаа';
      messageError(msg);
    }
    return null;
  }
